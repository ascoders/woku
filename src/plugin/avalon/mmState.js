define(["mmPromise", "mmRouter"], function () {
    ...
    var _root, undefine, _controllers = {},
        _states = {}
        /*
    ...
    var mmState = window.mmState = {
            ...
            popOne: function (chain, params, callback, notConfirmed) {
                if (mmState._toParams !== params) return callback(false, {
                    type: "abort"
                })
                var cur = chain.pop(),
                    me = this
            ...
            transitionTo: function (fromState, toState, toParams, options) {
                ...
                if (info && avalon.history) avalon.history.updateLocation(info.path + info.query, avalon.mix({}, options, {
                    silent: true
                }), !fromState && location.hash)
