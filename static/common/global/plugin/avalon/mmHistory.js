...
define(["avalon"], function (avalon) {
    ...
    History.prototype = {
        ...
        _getHash: function _getHash(path) {
            if (path.indexOf("#/") === 0) {
        ...
        _getAbsolutePath: function _getAbsolutePath(a) {
            return !a.hasAttribute ? a.getAttribute("href", 4) : a.href;
        },
        start: function start(options) {
            if (History.started) throw new Error("avalon.history has already been started");
        ...
        fireRouteChange: function fireRouteChange(hash, options) {
        ...
        updateLocation: function updateLocation(hash, options, urlHash) {
            var options = options || {},
                rp = options.replace,
                st = options.silent;
            if (this.monitorMode === "popstate") {
        ...
        _setHash: function _setHash(location, hash, replace) {
            var href = location.href.replace(/(javascript:|#).*$/, "");
            if (replace) {
                location.replace(href + hash);
            } else location.hash = hash;
        },
        _setIframeHistory: function _setIframeHistory(hash, replace) {
