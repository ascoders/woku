...
define(["mmHistory"], function () {
    ...
    Router.prototype = {
        ...
        _pathToRegExp: function _pathToRegExp(pattern, opts) {
            var keys = opts.keys = [],
        ...
        //添加一个路由规则
        add: function add(method, path, callback, opts) {
        ...
        route: function route(method, path, query) {
            path = path.trim();
        ...
        _parseArgs: function _parseArgs(match, stateObj) {
        ...
        setLastPath: function setLastPath(path) {
        ...
        */
        navigate: function navigate(hash, options) {
            var parsed = parseQuery((hash.charAt(0) !== "/" ? "/" : "") + hash),
                options = options || {};
            if (hash.charAt(0) === "/") hash = hash.slice(1); // 修正出现多扛的情况 fix http://localhost:8383/index.html#!//
        ...
        */
        when: function when(path, redirect) {
            var me = this,
        ...
        */
        ...
        urlFormate: function urlFormate(url, params, query) {
