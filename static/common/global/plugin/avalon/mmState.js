...
define("mmState", ["mmPromise", "mmRouter"], function () {
    ...
    */
    ...
    var mmState = window.mmState = {
        ...
        popOne: function popOne(chain, params, callback) {
            var cur = chain.pop(),
                me = this;
        ...
        pushOne: function pushOne(chain, params, callback, _local, toLocals) {
            var cur = chain.shift(),
                me = this;
            // 退出
            if (!cur) {
        ...
        transitionTo: function transitionTo(fromState, toState, toParams, options) {
    ...
    function broadCastBeforeUnload(exitChain, enterChain) {
        ...
        var end = enterChain[0].stateName,
            tmp,
            state;
    ...
    avalon.state = function (stateName, opts) {
        ...
        avalon.router.get(state.url, function (params, _local) {
            ...
            avalon.each(state.views, function (name, view) {
                ...
                var prom,
                    callback = function callback($ctrl) {
    ...
    */
    ...
    StateModel.prototype = {
        formate: function formate(options) {
        ...
        watch: function watch(eventName, func) {
            var events = this.events[eventName] || [];
            this.events[eventName] = events;
            events.push(func);
            return func;
        },
        fire: function fire(eventName, state) {
            var events = this.events[eventName] || [],
                i = 0;
            while (events[i]) {
        ...
        unwatch: function unwatch(eventName, func) {
            var events = this.events[eventName];
            if (!events) return;
            var i = 0;
            while (events[i]) {
        ...
        paramsChanged: function paramsChanged(toParams) {
            var changed = false,
                keys = this.keys,
                me = this,
                params = this.params;
        ...
        filterParams: function filterParams(toParams) {
        ...
        syncParams: function syncParams(toParams) {
            var me = this;
